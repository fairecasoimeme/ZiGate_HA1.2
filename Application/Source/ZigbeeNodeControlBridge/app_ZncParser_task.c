...
OS_TASK(APP_taskZncb)
{
    ...
    if (OS_E_OK == OS_eCollectMessage(APP_msgZpsEvents, &sStackEvent)) {
        ...
        case ZPS_EVENT_APS_DATA_CONFIRM:
            ...
            vSL_WriteMessage(E_SL_APS_DATA_CONFIRM, 4, au8values,u8LinkQuality);
        break;
#endif
#ifdef APP_APS_ACK
        case ZPS_EVENT_APS_DATA_ACK:
            ...
            vSL_WriteMessage(E_SL_APS_DATA_ACK, 8, au8values,u8LinkQuality);
        break;
#endif
            ...
            case ZPS_EVENT_APS_DATA_INDICATION:
            {
                        ...
                        sStackEvent.uEvent.sApsDataIndEvent.u8DstEndpoint != 0)
                {
                    ...
                    vSL_WriteMessage(E_SL_MSG_DATA_INDICATION, u16Length, au8StatusBuffer,u8LinkQuality);
     ...
     /* Not pushing the sequence number */
                         ...
                         u16Length += sizeof(uint8);
                         vSL_WriteMessage(E_SL_MSG_DEVICE_ANNOUNCE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                        break;
                    ...
                    case 0x8002:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sNodeDescRsp.sNodeDescriptor.uBitUnion.u16Value,sizeof(uint16) );
                         u16Length += sizeof(uint16);
                         pu8Buffer += sizeof(uint16);
                        vSL_WriteMessage(E_SL_MSG_NODE_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
					break;
                    ...
                    case 0x8006:
                    {
                        ...
                        vSL_WriteMessage(E_SL_MSG_MATCH_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
					break;
                    ...
                    case 0x8532:
					{
						...
						vSL_WriteMessage(E_SL_MSG_USER_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
					}
					break;
                    ...
                    case 0x8533:
					{
						...
						vSL_WriteMessage(E_SL_MSG_USER_DESCRIPTOR_SET_CONFIRM, u16Length, au8StatusBuffer,u8LinkQuality);
					}
					break;
					...
					case 0x8531:
					{
						...
						vSL_WriteMessage(E_SL_MSG_COMPLEX_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
					}
					break;
                    ...
                    case 0x8004:
                    {
                        ...
                        vSL_WriteMessage(E_SL_MSG_SIMPLE_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                        break;
                    ...
                    case 0x8001:
                    {
                        ...
                        {
                            vSL_WriteMessage(E_SL_MSG_NETWORK_ADDRESS_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                        }
                        else
                        {
                            vSL_WriteMessage(E_SL_MSG_IEEE_ADDRESS_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    ...
                    case 0x8034:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sMgmtLeaveRsp.u8Status,sizeof(uint8) );
                         u16Length += sizeof(uint8);
                         pu8Buffer += sizeof(uint8);
                        vSL_WriteMessage(E_SL_MSG_MANAGEMENT_LEAVE_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                    break;
                    ...
                    case 0x8031:
                    {
                        ...
                        for(u8Values = 0; u8Values < sApsZdpEvent.uZdpData.sMgmtLqiRsp.u8NeighborTableListCount; u8Values++)
                        {
                          ...
                          vSL_WriteMessage(E_SL_MSG_MANAGEMENT_LQI_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                        break;
                    case 0x8003:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sPowerDescRsp.u8Status,sizeof(uint8) );
                         ...
                         u16Length += sizeof(uint16);
                         pu8Buffer += sizeof(uint16);
                         vSL_WriteMessage(E_SL_MSG_POWER_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                    break;
                    ...
                    case 0x8005:
                    {
                         ...
                         vSL_WriteMessage(E_SL_MSG_ACTIVE_ENDPOINT_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    ...
                    case 0x8038:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sMgmtNwkUpdateNotify.u8Status,sizeof(uint8) );
                         ...
                         vSL_WriteMessage(E_SL_MSG_MANAGEMENT_NETWORK_UPDATE_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                    break;
                    case 0x8015:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sSystemServerDiscoveryRsp.u16ServerMask,sizeof(uint16) );
                         u16Length += sizeof(uint16);
                         pu8Buffer += sizeof(uint16);
                         vSL_WriteMessage(E_SL_MSG_SYSTEM_SERVER_DISCOVERY_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                    }
                    break;
                    default:
                    {
                        ...
                        memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUnbindRsp.u8Status,sizeof(uint8) );
                         ...
                         {
                         case 0x8021:
                            vSL_WriteMessage(E_SL_MSG_BIND_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                            break;
                         case 0x8022:
                            vSL_WriteMessage(E_SL_MSG_UNBIND_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
                            break;
			...
			case ZPS_EVENT_NWK_ROUTE_DISCOVERY_CONFIRM:
				...
				u16Length += sizeof(uint8);
				vSL_WriteMessage(E_SL_MSG_ROUTE_DISCOVERY_CONFIRM, u16Length, au8StatusBuffer,u8LinkQuality);
			break;
    ...
    if(sAppEvent.eType == APP_E_EVENT_SEND_PERMIT_JOIN)
    {
        ...
        vSL_WriteMessage(E_SL_MSG_GET_PERMIT_JOIN_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
    }
    ...
    if(sAppEvent.eType == APP_E_EVENT_NETWORK_STATE)
    {
        ...
        *pu8Buffer = (uint8)u32Channel;
        ...
        vSL_WriteMessage(E_SL_MSG_NETWORK_STATE_RSP, u16Length, au8StatusBuffer,u8LinkQuality);
    }
    ...
    if(sAppEvent.eType == APP_E_EVENT_ENCRYPT_SEND_KEY)
    {
        ...
        u16Length += sizeof(uint64) ;
        pu8Buffer += sizeof(uint64) ;
        vSL_WriteMessage(E_SL_MSG_AUTHENTICATE_DEVICE_RESPONSE, u16Length, au8StatusBuffer,u8LinkQuality);
    }
    ...
    case E_RUNNING:
        if (sStackEvent.eType != ZPS_EVENT_NONE) {
            ...
            if (sStackEvent.eType == ZPS_EVENT_NWK_NEW_NODE_HAS_JOINED) {
                ...
                u8JoinedDevice = 0;
                u16Length =  0;
                memcpy(pu8Buffer,&sStackEvent.uEvent.sNwkJoinIndicationEvent.u16NwkAddr,sizeof(uint16));
                u16Length=sizeof(uint16);
                pu8Buffer +=sizeof(uint16);
                memcpy(pu8Buffer,&sStackEvent.uEvent.sNwkJoinIndicationEvent.u64ExtAddr,sizeof(uint64));
                u16Length+=sizeof(uint64);
                pu8Buffer +=sizeof(uint64);
                memcpy(pu8Buffer,&sStackEvent.uEvent.sNwkJoinIndicationEvent.u8Capability,sizeof(uint8));
				u16Length+=sizeof(uint8);
				pu8Buffer +=sizeof(uint8);
				...
				/* GESTION DES PLUZZY UNSECURE ZHA 1.1*/
            }
            ...
            if (sStackEvent.eType == ZPS_EVENT_NWK_LEAVE_INDICATION) {
                    ...
                    u16Length += sizeof(uint8);
                    pu8Buffer += sizeof(uint8);
                    vSL_WriteMessage(E_SL_MSG_LEAVE_INDICATION, u16Length, au8StatusBuffer,u8LinkQuality);
            }
        }
...
PUBLIC void Znc_vSendDataIndicationToHost(ZPS_tsAfEvent *psStackEvent,uint8* pau8StatusBuffer)
{
    ...
    uint8 u8LinkQuality=psStackEvent->uEvent.sApsDataIndEvent.u8LinkQuality;
    ...
    memcpy(pu8Buffer,dataPtr,(sizeof(uint8)*u8Size) );
    u16Length += (sizeof(uint8)*u8Size);
    vSL_WriteMessage(E_SL_MSG_DATA_INDICATION, u16Length, pau8StatusBuffer,u8LinkQuality);
}
/****************************************************************************/
/***        END OF FILE                                                   ***/
/****************************************************************************/
